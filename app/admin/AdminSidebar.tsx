...
function NotesIoButtons({ onImported }: { onImported?: () => void }) {
  ...
  const showToast = useCallback((m: string) => {
    ...
    setToastLeaving(false); setToast(m)
    timerRef.current = setTimeout(() => {
      setToastLeaving(true)
      leaveRef.current = setTimeout(() => { setToast(null); setToastLeaving(false) }, 280)
    }, 1800)
  }, [])
  async function doExport() {
    ...
    setBusy('export')
    try {
      const r = await fetch('/api/admin/notes/export', { cache: 'no-store' })
      ...
      a.href = url
      a.download = `ixi-notes-${new Date().toISOString().slice(0, 10)}.json`
      document.body.appendChild(a); a.click(); a.remove()
      URL.revokeObjectURL(url)
      showToast(`Exported ${j.count ?? j.notes?.length ?? 0} notes ✓`)
  ...
  async function doImport(mode: 'merge' | 'replace') {
    ...
    try {
      ...
      const qs = mode === 'replace' ? '?confirm=1&mode=replace' : '?mode=merge'
      const r = await fetch(`/api/admin/notes/import${qs}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...((parsed as any) && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed as any : { notes: parsed }), mode }) })
      const j = await r.json().catch(() => ({}))
      if (!r.ok) throw new Error(j.error || 'Import failed')
      showToast(`Imported ${j.inserted ?? 0} notes ✓`)
      if (typeof window !== 'undefined') window.dispatchEvent(new Event('notes:changed'))
      onImported?.()
    } catch (e) { showToast(e instanceof Error ? e.message : 'Import failed') }
    finally { setBusy(null); setPendingFile(null) }
  }
  return (
    <>
      <div className="flex gap-1">
        <button
          onClick={doExport}
          disabled={!!busy}
          className="flex-1 flex items-center gap-1.5 px-2.5 py-1.5 text-[11px] font-mono border border-zinc-800 bg-zinc-900 text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800 hover:border-zinc-700 disabled:opacity-50 transition-colors"
          title="Export all notes as JSON (hierarchy preserved)"
        >
          <Download className="h-3 w-3 shrink-0" /> {busy === 'export' ? '…' : 'Export'}
        </button>
        <button
        ...
        >
          <Upload className="h-3 w-3 shrink-0" /> {busy === 'import' ? '…' : 'Import'}
        </button>
        <input ref={fileRef} type="file" accept=".json,application/json" className="hidden" onChange={onPickFile} />
      </div>
      {confirmOpen && pendingFile && typeof document !== 'undefined' ? createPortal(
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
          <button aria-label="close" onClick={() => { setConfirmOpen(false); setPendingFile(null) }} className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
          <div className="relative w-full max-w-sm bg-zinc-950 border border-zinc-800 shadow-2xl p-5">
            <div className="text-sm font-bold text-zinc-100 mb-1 font-mono">Import notes</div>
            <div className="text-xs text-zinc-500 font-mono mb-3 break-all">{pendingFile.name} · {(pendingFile.size / 1024).toFixed(1)} KB</div>
            <p className="text-xs text-zinc-400 font-mono mb-4">Merge keeps existing notes. Replace deletes all current notes first (requires confirmation).</p>
            <div className="flex gap-2">
              <button onClick={() => doImport('merge')} className="flex-1 py-2 bg-zinc-100 text-zinc-900 text-xs font-mono border border-zinc-100 hover:bg-white">Merge</button>
              <button onClick={() => doImport('replace')} className="flex-1 py-2 bg-red-950/40 text-red-300 text-xs font-mono border border-red-900/50 hover:bg-red-950/60">Replace all</button>
            </div>
            <button onClick={() => { setConfirmOpen(false); setPendingFile(null) }} className="w-full mt-2 py-2 border border-zinc-800 text-zinc-400 hover:bg-zinc-900 hover:text-zinc-200 text-xs font-mono">Cancel</button>
          </div>
...
function MobileSidebarInner({ onNavigate, onLogout }: { onNavigate: () => void; onLogout: () => void }) {
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="pt-3 mt-1 border-t border-zinc-800 space-y-1">
        <div className="text-[10px] font-mono text-zinc-600 px-2 py-1 tracking-widest uppercase">Notes data</div>
        <NotesIoButtons />
      </div>
