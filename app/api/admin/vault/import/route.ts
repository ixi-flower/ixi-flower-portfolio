...
function parseCSV(text: string): ImportEntry[] {
  // minimal CSV: title,site,username,password,notes — first row may be header
  ...
  const split = (line: string) =>
    line.match(/(".*?"|[^",]+|(?<=,)(?=,))/g)?.map(s => s.replace(/^"|"$/g, "").replace(/""/g, '"').trim()) ?? []
...
export async function POST(request: NextRequest) {
  ...
  if (contentType.includes("application/json")) {
    ...
    // try JSON first, fallback CSV
    const text = await request.text()
    try {
      const parsed = JSON.parse(text)
      rawEntries = normalizeImport(parsed)
  ...
  if (rawEntries.length === 0) return NextResponse.json({ error: "No entries found — send { entries: [{title,password,site?,username?,notes?}] } or CSV with header title,site,username,password,notes" }, { status: 400 })
  if (rawEntries.length > 500) return NextResponse.json({ error: "Too many entries (max 500 per import)" }, { status: 400 })
  ...
  for (let i = 0; i < rawEntries.length; i++) {
    ...
    valid.push({
      title,
      site: (r as any).site != null ? String((r as any).site).trim().slice(0, 500) || null : null,
      username: (r as any).username != null ? String((r as any).username).trim().slice(0, 240) || null : null,
      password,
      notes: (r as any).notes != null ? String((r as any).notes).trim().slice(0, 500) || null : null,
    })
  }
  if (errors.length > 0) {
  ...
  if (mode === "replace") {
    // destructive: require explicit confirmation via body.confirmReplace === true
    const confirmBody = contentType.includes("application/json") ? null : null // already consumed
    // We gate by checking the request had ?confirm=1 or body.confirmReplace
    // Re-read: for JSON the caller should include confirmReplace:true
    // For safety also check query param
    const qConfirm = request.nextUrl.searchParams.get("confirm")
    let bodyConfirm = false
    try {
      // we already parsed body above only as rawEntries; re-check not possible, so rely on qConfirm for JSON replace
      bodyConfirm = qConfirm === "1"
    } catch {}
    if (!bodyConfirm) {
      // Also accept if original JSON body had confirmReplace:true — we lost it after consuming stream,
      // so for JSON replace the client must use ?confirm=1 as well.
      return NextResponse.json({ error: 'Replace mode requires ?confirm=1 — this deletes all existing vault entries. Use merge to keep existing.' }, { status: 400 })
    }
    await db.delete(vaultEntries)
  }

  // insert — preserve order from file; assign sortOrder starting after existing max (merge) or 0 (replace)
  let startOrder = 0
  if (mode === "merge") {
  ...
  return NextResponse.json({ ok: true, mode, inserted, skipped: rawEntries.length - valid.length })
}
